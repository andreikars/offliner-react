...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    const fetchCartItems = async () => {
      try {
        const response = await axios.get(`https://5eb6-93-84-5-230.ngrok-free.app/api/cart?userId=${userId}`);
  ...
  const handleQuantityChange = async (cartItemId, newQuantity) => {
    ...
    try {
      ...
      await axios.put(`https://5eb6-93-84-5-230.ngrok-free.app/api/cart/${cartItemId}`, { quantity });
  ...
  const handleRemoveItem = async (cartItemId) => {
    try {
      await axios.delete(`https://5eb6-93-84-5-230.ngrok-free.app/api/cart/${cartItemId}`);
