...
export default function Home() {
  ...
  const loadProducts = async () => {
    try {
      let url = 'https://tidy-hornets-exist.loca.lt/api/products/all';
      if (selectedCategory) {
        console.log(`Fetching products with category ID: ${selectedCategory.id}`);
        url = `https://tidy-hornets-exist.loca.lt/api/products?categoryId=${selectedCategory.id}`;
      }
  ...
  const handleAddToCart = async (productId) => {
    ...
    try {
      ...
      await axios.post('https://tidy-hornets-exist.loca.lt/api/cart', cartItem);
          ...
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product) => (
              <div className="col-md-4 mb-4" key={product.id}>
                <div className="card">
                  <img
                    src={`https://tidy-hornets-exist.loca.lt${product.imageUrl}`} // Путь, начинающийся с /product/
